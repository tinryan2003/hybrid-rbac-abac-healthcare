...
const columns = [
  ...
  { header: "Date", accessor: "appointmentDate", className: "hidden md:table-cell" },
  { header: "Time", accessor: "appointmentTime", className: "hidden lg:table-cell" },
...
const EventListPage = () => {
  ...
  const renderRow = (item: AppointmentDto) => (
    ...
    >
      <td className="flex items-center gap-4 p-4">{item.patientName || `Patient #${item.patientId ?? ""}`}</td>
      ...
      <td className="hidden lg:table-cell p-4">{item.appointmentTime ?? "—"}</td>
      <td className="hidden lg:table-cell p-4 max-w-[180px] truncate" title={item.reason ?? ""}>
        {item.reason ?? "—"}
      </td>
      <td className="p-4">
        <span
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
      <div className="flex items-center justify-between">
        <h1 className="hidden md:block text-lg font-semibold">Appointments</h1>
