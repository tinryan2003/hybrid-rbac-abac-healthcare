    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(authz -> authz
                        // Public endpoints (gateway strips /api so paths are /patients/...)
                        .requestMatchers(
                                "/actuator/health",
                                "/actuator/info",
                                "/patients/health",
                                "/patients/keycloak/**",  // For inter-service calls
                                "/patients/pip/**"        // PIP - resource attributes for authorization-service
                        ).permitAll()
                        
                        // Patient API (behind gateway: /api/patients -> /patients)
